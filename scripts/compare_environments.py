...
def plot_reward_comparison(results: Dict):
    ...
    try:
        ...
        ax1.hist(original_rewards, alpha=0.7, label="原始环境", bins=10)
        ax1.hist(improved_rewards, alpha=0.7, label="改进环境", bins=10)
        ax1.set_xlabel("总奖励")
        ax1.set_ylabel("频次")
        ...
        ax2.bar(x - width/2, original_vals, width, label="原始环境", alpha=0.7)
        ax2.bar(x + width/2, improved_vals, width, label="改进环境", alpha=0.7)
        ax2.set_ylabel("指标值")
        ax2.set_title("关键指标对比")
        ax2.set_xticks(x)
        ...
        plt.tight_layout()
        plt.savefig("/workspace/environment_comparison.png", dpi=150, bbox_inches='tight')
        print(f"\n图表已保存到: /workspace/environment_comparison.png")
